...
sanitizeToLatex <- function(str, html=FALSE, type=c("text","table")) {
      ...
      result <- gsub("ó","\\'{o}",result,fixed=TRUE)
      result <- gsub("ò","\\`{o}",result,fixed=TRUE)
      result <- gsub("í","\\'{i}",result,fixed=TRUE)
      result <- gsub("ì","\\`{i}",result,fixed=TRUE)
      result <- gsub("Í","\\'{I}",result,fixed=TRUE)
      result <- gsub("Ì","\\`{I}",result,fixed=TRUE)      
      result <- gsub("Å","\\r{A}",result,fixed=TRUE)
      result <- gsub("ç","\\c{c}",result,fixed=TRUE)
      result <- gsub("á","\\'{a}",result,fixed=TRUE)
      result <- gsub("à","\\`{a}",result,fixed=TRUE)
      result <- gsub("Á","\\'{A}",result,fixed=TRUE)
      result <- gsub("À","\\`{A}",result,fixed=TRUE)
      result <- gsub("ú","\\'{u}",result,fixed=TRUE)
      result <- gsub("ù","\\`{u}",result,fixed=TRUE)
      result <- gsub("ñ","\\~{n}",result,fixed=TRUE)
